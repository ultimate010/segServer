#ifndef WOODPECKER_DAEMON_H
#define WOODPECKER_DAEMON_H
#include <stdio.h>
#include <string.h>
#include <errno.h>
#include <unistd.h>
...
namespace segServer
{
    ...
    {
        public:
            Daemon(HuskyServer * workHandler, const char* pidFile)
            {
        ...
        public:
            ...
            {
                string masterPidStr;
                loadFile2Str(m_pidFile, masterPidStr);
    ...
    HuskyServer * Daemon::m_pHandler;
    int Daemon::m_nChildPid = 0;

    void Daemon::sigMasterHandler(int sig)
    {
        kill(m_nChildPid,SIGUSR1);
        LogDebug("master = %d sig child =%d!",getpid(),m_nChildPid); //当前进程pid
